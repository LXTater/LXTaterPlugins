...
return function()
  ...
  --- Uses offset from sourceStart to preserve gaps/spacing in the destination
  local function buildCopyPreview(items, sourceStart, destStart, value, isPrefix, doStripAuto, cmdKeyword)
    local lines = {}
    for _, item in ipairs(items) do
  ...
  --- Copy source range to destination in one bulk command (preserves gaps/empty slots),
  --- then relabel destination items using the ORIGINAL source names + prefix/suffix.
  --- No pool re-read needed — we know where everything lands from the offsets.
  --- MA3 syntax: Copy <type> <start> Thru <end> At <type> <destStart>
  ---
  --- After copy, MA may auto-rename items in two ways:
  ---   "Group 1"  → copied as "Group 2"    (MA increments trailing number)
  ---   "Group1"   → copied as "Group1 #2"  (MA appends " #2")
  --- We overwrite whatever MA named the copy using the original source name
  --- plus the user's prefix/suffix.
  local function applyCopyAndRelabel(items, sourceStart, sourceEnd, destStart, value, isPrefix, doStripAuto, cmdKeyword)
    local labelCount = 0

    -- ── Step 1: Single bulk copy (runs ONCE) ──
    local copyCmd = string.format('Copy %s %d Thru %d At %s %d', cmdKeyword, sourceStart, sourceEnd, cmdKeyword, destStart)
    msg(copyCmd)
    local copyOk = safeCmd(copyCmd)

    if not copyOk then
      err("Bulk copy command failed. Aborting relabel.")
      return 0, 0
    end

    msg(string.format("Copy complete. Now relabeling %d item(s)...", #items))

    -- ── Step 2: Label each destination slot using ORIGINAL source name + offset ──
    -- No need to re-read the pool — we know exactly where each item lands.
    for _, item in ipairs(items) do
      local destSlot = destStart + (item.no - sourceStart)
      local newName = computeNewName(item.name, value, isPrefix, doStripAuto)
  ...
  local function confirmCopyPreview(items, sourceStart, sourceEnd, destStart, value, isPrefix, doStripAuto, displayName, cmdKeyword)
    local mode = isPrefix and "Prefix" or "Suffix"
    local stripNote = doStripAuto and "Auto-numbers WILL be stripped.\n\n" or ""
    local rangeSpan = sourceEnd - sourceStart
    local destEnd = destStart + rangeSpan
    local previewStr = buildCopyPreview(items, sourceStart, destStart, value, isPrefix, doStripAuto, cmdKeyword)
  ...
  local function pickDestination(displayName, itemCount, rangeSpan)
    local result = MessageBox({
      title = "TaterRelabel — Destination",
      icon = "object_smart",
      titleTextColor = "Global.Focus",
      message = string.format(
        "You are copying %d %s item(s) across a span of %d slots.\n\n"
        .. "Gaps/empty slots will be preserved at their relative positions.\n\n"
        .. "Enter the STARTING slot number for the destination.\n"
        .. "Destination will span from <start> through <start>+%d.",
        itemCount, displayName, rangeSpan + 1, rangeSpan
      ),
      commands = {
        { value = 1, name = "Continue" },
        { value = 0, name = "Cancel" },
  ...
  local function mainMenu()
    ...
    local destStart = nil
    local rangeSpan = endNum - startNum  -- total span of the source range (preserving gaps)
    if isCopyMode then
      destStart = pickDestination(displayName, #items, rangeSpan)
      if not destStart then
        msg("Cancelled.")
        return
      end
    ...
    if isCopyMode then
      -- Copy & Rename flow
      local confirmed = confirmCopyPreview(items, startNum, endNum, destStart, value, isPrefix, doStripAuto, displayName, cmdKeyword)
      if not confirmed then
        msg("Cancelled.")
        return
      end

      local copyCount, labelCount = applyCopyAndRelabel(items, startNum, endNum, destStart, value, isPrefix, doStripAuto, cmdKeyword)
      local destEnd = destStart + rangeSpan
