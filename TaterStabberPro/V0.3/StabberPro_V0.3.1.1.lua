...
-- Author: LXTater
-- Version: 0.3.1.1
-- NOTE: Test and debug only. Backup your showfile first! -- USE VERSION 0.3.1.0 
...
local editSequEnabled = true -- default
 local function openSettings(caller) 
      local returnTable = MessageBox(
            ...
            --[[  states = {{
                ...
                {
                name = 'Edit Sequence GUI on Build',
                selectedValue = 1,
                type = 1,
           ...
           -- ModeString = SettingsSelectors{2}
             -- Set state.groupStr from SettingsInputs[1]
                      if SettingsInputs and SettingsInputs[1] then
                          state.groupStr = tostring(SettingsInputs[1])
                      end

                      -- Set state.selectedMode from SettingsSelectors[1]
                      if SettingsSelectors and SettingsSelectors[1] then
                          state.selectedMode = SettingsSelectors[1]
                      end

                      -- Set editSequEnabled from SettingsSelectors[2]
                      if SettingsSelectors and SettingsSelectors[2] then
                          editSequEnabled = (SettingsSelectors[2])
                      end
            if returnTable then
