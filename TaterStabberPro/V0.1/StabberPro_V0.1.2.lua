...
-- cues and recipe lines based on detected MAtricks (focusing on X/Y Group, Block, Wings) and presets.
--
-- Key Features:
...
--   - Uses selection grid dimensions (X/Y) for number of cues, overriding to linear if not active and 1D.
--   - Each cue represents a grid position combination, with one part per preset.
--   - Sets MAtricks properties X/Y for positions (normalized to start from 1); sets Group/Block/Wings if defined (> 0).
--
-- Limitations:
