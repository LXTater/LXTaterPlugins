...
return function()
  ...
  local function openPresetPicker()
    ...
    -- child window
    local pop = overlay:Append("BaseInput"); pop.Name = "PresetPicker"; pop.W = 1200; pop.H = 0; pop.Columns=1; pop.Rows=2; pop[1][1].SizePolicy="Stretch"; pop[1][1].Size="60"; pop[1][2].SizePolicy="Fixed"; pop[1][2].Size="600"; pop.CloseOnEscape="Yes"
    local tb = pop:Append("TitleBar"); tb.Columns=2; tb.Rows=1; tb[2][2].SizePolicy="Stretch"; tb[2][2].Size="50"; tb.Texture="corner2"
    ...
    frame[1][1].SizePolicy = "Stretch"; --frame[1][1].Size = 200
    frame[2][1].SizePolicy = "Stretch"; --frame[2][1].Size = 1000

    local container = frame:Append("DialogContainer"); container.Name = "tab_contents"; container.anchors = '1,0'

    local tab = frame:Append("UITab"); tab.Name = "presetTabs"; tab.Type = "Vertical"; tab.Texture = "corner5"; tab.PluginComponent = myHandle; tab.TabChanged="OnPresetTabChanged"; tab.H = 600; tab.W = 200; tab.ItemSize = 60
    tab:WaitInit()
    ...
    if pools then
      for i=1,#pools do
        ...
        if pool then
          tabCount = tabCount + 1
          local grid = container:Append("UILayoutGrid"); grid.Margin = {left=10,right=10,top=10,bottom=10}; grid.Rows=30; grid.Columns=2; grid.Name = "pool"..i
          local children = pool:Children() or {}
          local r,c = 0,0
          for _,pr in ipairs(children) do
            if pr then
              local btn = grid:Append("Button")
              local idx = pr.no or (Obj.Index(pr) + 1)
    ...
    tab.SelectedItem = 1
    signalTable.OnPresetTabChanged(tab)
  end
