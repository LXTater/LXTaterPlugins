...
-- matching groups, referenced presets, and MAtricks state. Additionally, creates a temporary sequence
-- with blank cues and recipe lines based on the detected MAtricks (focusing on XGroup) and presets.
-- 
-- Key Features:
...
--   - Sets MAtricks properties X and XGroup on each part.
-- 
-- Limitations:
...
-- - Focuses on XGroup for MAtricks; other parameters (XWings, XBlocks, Y versions) not handled yet.
...
return function()
  ...
  local function stabberRecipeMAtricksProg(poolItem)
    Echo("Start Function recipeMAtricksProg")
    local MAtrickscount = DataPool().MAtricks:Children()
    local MAtricks = DataPool().MAtricks
    local lastMAtricks = #MAtrickscount
    local helperMAtricksindex
    if lastMAtricks == 0 then
      helperMAtricksindex = 9000
    else
      helperMAtricksindex = MAtrickscount[lastMAtricks].index + 1
      if helperMAtricksindex < 800 then helperMAtricksindex = helperMAtricksindex + 9000 end
    end
    local helperMAtricksname = "HelperMAtricksStabber"
    local MAtricksPreset = nil

    Cmd("Store Matricks " .. helperMAtricksindex .. " '" .. helperMAtricksname .. "'")
    Cmd("Reset Selection MAtricks")

    local referenceMAtricks = MAtricks[helperMAtricksindex]

    if poolItem then
      for i = 1, #MAtricks - 1 do
        local currentMAtricks = MAtricks[i]
        if referenceMAtricks:Compare(currentMAtricks) and (currentMAtricks.index) ~= helperMAtricksindex then
          MAtricksPreset = currentMAtricks.index
          helperMAtricksindex = nil
          Echo("Found MAtricks " .. currentMAtricks.index)
          break
        end
      end
      if MAtricksPreset == nil then
        MAtricksPreset = ToAddr(Selection().InitialMAtricks)
        if MAtricksPreset ~= nil then
          helperMAtricksindex = nil
          Echo("Found MAtricks in " .. MAtricksPreset)
        end
      end
    else
      Echo("MAtricks Helper Index " .. helperMAtricksindex)
    end

    Echo("End Function recipeMAtricksProg")
    return helperMAtricksindex, MAtricksPreset
  end
  ...
  -- Fetch World (if any) - assuming from original logic, but simplified here as not implemented; set to nil if none
  ...
  if stabberMI.active then
    local poolItem = (stabberMI.source == "Pool")
    stabberHelperIndex, _ = stabberRecipeMAtricksProg(poolItem)
  ...
  -- Determine number of slices and group value
  local stabberG = (stabberXGroup > 1) and stabberXGroup or stabberSelCount
  local stabberSlices = stabberG
  ...
  for stabberK = 1, stabberSlices do
    ...
    for stabberP = 1, #stabberPresets do
      ...
      Cmd("Set Sequence \"StabberRecipeTemp\" Cue " .. stabberK .. " Part 0." .. stabberY .. " Property \"XGroup\" " .. stabberG)
    end
  end
