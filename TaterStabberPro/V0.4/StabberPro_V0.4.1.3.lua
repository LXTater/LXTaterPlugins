...
right.Columns = 3; right.Rows = 8
right.BackColor = colorTransparent
right.Anchors = {left=1,right=1,top=0,bottom=0}

-- Row 0: Preset entry + Add button
local pEntry = right:Append("LineEdit")
pEntry.Anchors = {left=0,right=1,top=0,bottom=0}
pEntry.Content = ""
pEntry.MaxTextLength = 16
pEntry.VkPluginName = "TextInput"
pEntry.PluginComponent = myHandle

local pAdd = right:Append("Button")
pAdd.Text = "+ Add"
pAdd.PluginComponent = myHandle
pAdd.Clicked = "OnPresetAdd"
pAdd.Anchors={left=2,right=2,top=0,bottom=0}

-- Row 1-2: Preset list display
local pList = right:Append("Button")
pList.Anchors = {left=0,right=2,top=1,bottom=2}
pList.HasHover = "No"
...
maTricksLabel.Anchors = {left=0,right=0,top=4,bottom=4}

mX = right:Append("CheckBox")  -- Remove 'local', make it accessible to functions
mX.Text="X"
mX.State=1
mX.Anchors={left=1,right=1,top=4,bottom=4}  -- Changed from left=0 to left=1
mX.PluginComponent=myHandle
mX.Clicked="OnMaTricksX"

mY = right:Append("CheckBox")  -- Remove 'local'
mY.Text="Y"
mY.State=0
mY.Anchors={left=2,right=2,top=4,bottom=4}  -- Changed from left=1 to left=2
mY.PluginComponent=myHandle
mY.Clicked="OnMaTricksY"

-- Remove mZ since we only have 3 columns and need space for label + 2 checkboxes
-- Or create it in a second row if you want all 3 visible at once
...
-- Row 5: Block (Remove 'local', make accessible to updateMaTricksLabels)
lblBlock = mkMaTricksRow("XBlock", 5, "OnBlockSubtract", "OnBlockAdd", "OnBlockClick")

-- Row 6: Group
lblGroup = mkMaTricksRow("XGroup", 6, "OnGroupSubtract", "OnGroupAdd", "OnGroupClick")

-- Row 7: Wings
lblWings = mkMaTricksRow("XWings", 7, "OnWingsSubtract", "OnWingsAdd", "OnWingsClick")
  ------------------------------------------------------------
 ...
 local function previewCueCount()
  ...
  signalTable.OnBuild  = function()
    ...
    local vals = getCurrentMaTricksValues()
    local xBlock = vals.block
    local xGroup = vals.group
    local xWings = vals.wings
