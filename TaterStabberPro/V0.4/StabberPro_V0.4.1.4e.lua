...
return function()
  ...
  SelectedSeq = SelectedSequence() -- Grab the initial selected Sequence at plugin start
...
local function deleteEmptyCues(seqNum)
 ...
 local function openSettings(caller)   --Still actively working on the settings menu.
  ...
  groupStr       = " Selection",
  seqNumStr      = SelectedSeq.no or "201",
  seqName        = SelectedSeq.name or "Stabber Recipe",
  -- MAtricks values for X, Y, Z
  maTricksAxis   = "X",  -- current selected axis
  xBlock = 0, xGroup = 0, xWings = 0,
  yBlock = 0, yGroup = 0, yWings = 0,
  -- zBlock = 0, zGroup = 0, zWings = 0,  -- Z axis disabled
...
local function updateSeqStatus()
  ...
  if not seqNum or seqNum < 1 then
    ...
    seqStatusLabel.TextColor = 1.7  -- Yellow for invalid
    return
  end
  ...
  if seqExists(seqNum) then
    ...
    seqStatusLabel.TextAlignmentH = "Center"
    seqStatusLabel.TextColor = 1.31  -- Red for existing
  else
    seqStatusLabel.Text = "Sequ State: Empty"
    seqStatusLabel.TextAlignmentH = "Center"
    seqStatusLabel.TextColor = 1.32  -- Green for new/empty
  end
end

local function toggleSeqTracking(seqNum)
  ...
  -- Only apply tracking if checkbox is enabled (state.seqTracking == 1)
  if state.seqTracking ~= 1 then
  ...
  if not success2 then
    err("Failed to set tracking on Sequence " .. tostring(seqNum) .. ": " .. tostring(error2))
  else
    msg("Tracking enabled for Sequence " .. tostring(seqNum))
  end
end
...
local function setMaTricksAxis(axis)
  ...
  -- Update button colors based on selected axis
  local axisColor
  if axis == "X" then
    axisColor = colorMATx
  elseif axis == "Y" then
...
local chkTracking = form:Append("CheckBox")
chkTracking.Text=""
chkTracking.State=0
...
seqStatusLabel = form:Append("UIObject")
seqStatusLabel.Text = "Sequ State: Empty"
seqStatusLabel.TextalignmentH = "Left"
seqStatusLabel.Font = "Medium14"
seqStatusLabel.TextColor = "0,1,0"  -- Green by default
...
-- Row 5: Shuffle Seed
local editShuffleSeed = mkLine("Shuffle Seed", state.shuffleSeedStr, "OnShuffleSeedChanged", 0,3,5,5, true)

-- Row 6: Mode selector (Straight/Shuffle/Scatter)
mkLabel("Mode", 0,0,6,6)
local mStraight = form:Append("CheckBox")
mStraight.Text="Straight"
mStraight.State=1
mStraight.Anchors={left=1,right=1,top=6,bottom=6}
...
mShuffle.State=0
mShuffle.Anchors={left=2,right=2,top=6,bottom=6}
...
mScatter.State=0
mScatter.Anchors={left=3,right=3,top=6,bottom=6}
...
cuePreview = right:Append("Button")
cuePreview.Anchors = {left=3,right=3,top=2,bottom=2}
cuePreview.HasHover = "No"
cuePreview.TextalignmentH = "Center"
cuePreview.Text = "—"
...
if #state.presets == 0 then err("No presets entered."); status.Text = "Error: No presets"; return end
   ...
   if seqExists(seqNum) then
      ...
      if not safeCmd('Delete Sequence '..seqNum..' cue 1 thru /NC') then
    ...
    -- Prepare X/Y/Z values by mode (all use same sequence)
    ...
    for c = 1, totalCues do
      ...
      for p = 1, #state.presets do
        ...
        -- Always apply X value
        if not safeCmd('Set Sequence '..seqNum..' Cue '..c..' Part '..partStr..' Property "X" '..seq_values[c]) then status.Text="Error: set X"; return end
        
        -- Only apply Y value if Y MATricks is enabled
        if state.enableMaTricksY then
          if not safeCmd('Set Sequence '..seqNum..' Cue '..c..' Part '..partStr..' Property "Y" '..seq_values[c]) then status.Text="Error: set Y"; return end
        end
...
signalTable.OnSeqTracking = function() 
  state.seqTracking = (state.seqTracking == 1) and 0 or 1
