...
local function deleteEmptyCues(seqNum)
 ...
 local function openSettings(caller)   --Still actively working on the settings menu.
  ...
  local colorMATz            = CT.MATricks.BackgroundZ
  -- Terrible way to fix some bugs lmao
  local cuePreview, lblBlock, lblGroup, lblWings, mX, mY, mZ, enableX, enableY, enableZ
...
local function previewCueCount()
  ...
  if not state.enableMaTricksX and not state.enableMaTricksY and not state.enableMaTricksZ then
  ...
  if xGroup > 0 or yGroup > 0 or zGroup > 0 then
    totalCues = math.max(xGroup, yGroup, zGroup)
  else
    ...
    totalCues = math.max(xCues, yCues, zCues)
  end
...
-- Row 5: Shuffle Seed
mkLine("Shuffle Seed", state.shuffleSeedStr, "OnShuffleSeedChanged", 0,3,5,5, true)
   ...
   if seqExists(seqNum) then
    ...
    for c = 1, totalCues do
      ...
      for p = 1, #state.presets do
        ...
        -- Only apply Z value if Z MATricks is enabled
        if state.enableMaTricksZ then
