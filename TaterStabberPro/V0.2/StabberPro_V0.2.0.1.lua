...
return function()
 ...
 -- Simplified preset picker cause the OG one was giving me a mf headache.
local function openPresetPicker()
  ...
  local poolItems = {}
  for i = 1, #pools do
    if pools[i] then poolItems[#poolItems+1] = ("Pool %d"):format(i) end
  end
  if #poolItems == 0 then
    PopupInput{title = "Pick Preset", caller = pPick, items = {"No preset pools found"}}
    return
  end
  ...
  local poolNum = tonumber(poolChoice:match("(%d+)$"))
  ...
  local _, pick = PopupInput{
    title  = ("Pick Preset — Pool %d"):format(poolNum),
    caller = pPick,
    items  = presetItems,
