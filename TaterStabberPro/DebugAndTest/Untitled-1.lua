...
return function()
  ...
  -- Row 1: Group, Seq #
  mkLabel("Group", 0,1,0,0)
  local bGroup = form:Append("Button"); bGroup.Anchors={left=1,right=2,top=0,bottom=0}; bGroup.Text = (state.groupStr~="" and ("Group"..state.groupStr)) or "Select Group"; bGroup.PluginComponent=myHandle; bGroup.Clicked="OnOpenGroupPicker"
  ...
  local function openPresetPicker()
    ...
    -- child window
    local pop = overlay:Append("BaseInput"); pop.Name = "PresetPicker"; pop.W = 900; pop.H = 0; pop.Columns=1; pop.Rows=2; pop[1][1].SizePolicy="Fixed"; pop[1][1].Size="60"; pop[1][2].SizePolicy="Fixed"; pop[1][2].Size="520"; pop.CloseOnEscape="Yes"
    local tb = pop:Append("TitleBar"); tb.Columns=2; tb.Rows=1; tb[2][2].SizePolicy="Fixed"; tb[2][2].Size="50"; tb.Texture="corner2"
    local tbtn = tb:Append("TitleButton"); tbtn.Text = "Pick Preset"
    local tclose = tb:Append("CloseButton"); tclose.Anchors="1,0"; tclose.Texture="corner2"; tclose.Clicked = "OnCloseChildPopup"; tclose.PluginComponent=myHandle

    local frame = pop:Append("DialogFrame"); frame.Columns=2; frame.Rows=1; frame.Anchors={left=0,right=0,top=1,bottom=1}
    frame[1][1].SizePolicy = "Fixed"; frame[1][1].Size = 520
    frame[2][1].SizePolicy = "Fixed"; frame[2][1].Size = 320

    local container = frame:Append("DialogContainer"); container.Name = "tab_contents"; container.anchors = '1,0'

    local tab = frame:Append("UITab"); tab.Name = "presetTabs"; tab.Type = "Vertical"; tab.ItemSize = 90; tab.Texture = "corner5"; tab.PluginComponent = myHandle; tab.TabChanged="OnPresetTabChanged"; tab.H = 520; tab.W = 320
    tab:WaitInit()
    ...
    if pools then
      for i=1,#pools do
        ...
        if pool then
          tabCount = tabCount + 1
          local grid = container:Append("UILayoutGrid"); grid.Margin = {left=10,right=10,top=10,bottom=10}; grid.Rows=20; grid.Columns=2; grid.Name = "pool"..i
          local children = pool:Children() or {}
          local r,c = 0,0
          for _,pr in ipairs(children) do
            if pr then
              local btn = grid:Append("Button")
              local idx = pr.no or pr.index or pr.Id or 0
              btn.Text = string.format("%d.%d %s", i, idx, pr.name or "Preset")
              btn.Name = string.format("preset|%d|%d", i, idx)
              btn.PluginComponent = myHandle
              btn.Clicked = "OnChoosePreset"
              btn.Anchors = {left=c,right=c,top=r,bottom=r}
              c = c + 1; if c>1 then c=0; r=r+1 end
            end
          end
          tab:AddListStringItem("Pool "..i, grid.Name)
        end
      end
    end
  ...
  signalTable.OnPresetTabChanged = function(caller)
    ...
    local tab = caller
    local count = tab:GetListItemsCount()
    for i=1,count do
      local name = tab:GetListItemValueStr(i)
      local obj = overlay:FindRecursive(name, "UILayoutGrid")
      if obj then obj.Visible = (name == tab.SelectedItemValueStr) and "Yes" or "No" end
    end
  end
