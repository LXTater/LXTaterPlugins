...
function CreateInputDialog(displayHandle)  
    ...
    require 'gma3_debug'() -- this is to incorporate VSCode workflow with GMA3
...
------------------------------------------------------------------------------------------
  ...
  local colorHeader = Root().ColorTheme.ColorGroups.UIGrid.HeaderAltBackground -- Used for headers, a lighter-greener grey
  local colorSheetSelection = Root().ColorTheme.ColorGroups.Global.BackgroundSelected -- New color for recipe sheet, a medium grey
...
---------------------------------Row 2-----------------------------------------------------
-- Main Content Grid, splits the dialog into two sides, left is recipe, right is settings--
-------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------
  ...
  inputGroup.Padding = "5%,5%"  -- Reduced padding
  inputGroup.W = "100%"  -- Keep current width
  inputGroup.H = "40%"  -- Changed height to ~15
  inputGroup.PluginComponent = myHandle
  inputGroup.Clicked = "SelectGroup"
  inputGroup.BackColor = colorSheetSelection 
  ...
  inputPreset.Padding = "5%,5%"  -- Reduced padding
  inputPreset.W = "100%"  -- Keep current width
  inputPreset.H = "40%"  -- Changed height to ~10%
  ...
  inputPreset.BackColor = colorSheetSelection
  -- Add TextChanged handler if needed later

  local recipeLine = recipeGrid:Append("Button")
  recipeLine.Prompt = "New Recipe - Not Supported yet."
    recipeLine.TextalignmentH = "Center"
    recipeLine.Anchors = {left=1, right=0, top=1, bottom=0}
    recipeLine.Padding = "10,10"  -- Reduced padding
    recipeLine.W = "100%"  -- Keep current width
    recipeLine.H = "100%"  -- Changed height to ~10%
    recipeLine.PluginComponent = myHandle
    recipeLine.Clicked = "newRecipeLine"
    recipeLine.BackColor = colorBackground


--------------------------------------Main Column 2--------------------------------------------
------------------------------ Stabber Settings + More --------------------------------------
------------------------------------------------------------------------------------------

  -- Right side: Original Stab Me UI (renamed to stabsGrid) 205 thru
...
UpdateNumSteps(Interleave)
  ...
  signalTable.SelectGroup = function(caller)
    ...
    local _, choice = PopupInput{title = "Select Group", caller = caller, items = groupNames, selectedValue = caller.Text}
    if choice then
      caller.Text = choice
      -- Store the selected group name if needed, e.g., selectedGroup = choice
    end
  end
...
function Store_cue(seqnumber, cuenumber)
  Cmd('at seq 9999 cue 1')
  Cmd('store '.. seqnumber .. ' cue '..cuenumber .. '/o')
  --Cmd('Assign Sequence ' ..seqnumber.. ' cue '..cuenumber.. '/mode=release')
  Cmd('Set '..Storeseq..' cue '..cuenumber..'Property "Command" "Goto #['..Storeseq..'] cue Offcue')
  Cmd('Set '..Storeseq..' cue "Offcue" Property "Command" "Off #['..Storeseq..'] ')
end
...
function Create_Seq()
  Cmd("Grid 'Linearize' 'LeftToRight'")
  Cmd('delete '..Storeseq..' cue 2 thru /nc')
  
  --Cmd('Blindedit On')
  --Cmd('Edit seq 9999 cue 1')
  ...
  --Cmd('BlindEdit Off')
  Cmd('ClearAll')
  Cmd('set '..Storeseq..'Property Wraparound 1')
  Cmd('set '..Storeseq..'Property RestartMode 2')
  Cmd('set cue offcue property ')
  Cmd('set '..Storeseq..'Property Tracking 0')
  Cmd('Label '..Storeseq..' '..Label)
  
  Cmd('set '..Storeseq..' cue offcue CueDelay '..OffTime)
  

  Cmd('Set '..Storeseq..' cue offcue CueFade '..OffFade)
...
function Shuffle_Seq()
  ...
  Cmd('Store seq 9999 cue 1 /o')
  local randomInt = math.random( 126 )
  CheckMAtricks()
  for i = 1, randomInt, 1 do
    Cmd('Shuffle')
  end
  Cmd('store group 9999 /o')
  Cmd('Clear')
  Cmd('group 9999')
  if Direction == "<" then
    local cueStoreNum = 1
  ...
  Cmd('ClearAll')
  Cmd('Delete group 9999 /nc')
  Cmd("Delete Seq 9999 /nc")
