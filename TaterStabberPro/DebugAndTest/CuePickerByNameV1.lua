...
--- Updated by www.LXTater.com to utilize names over number for the argument!
--
-- License: 
...
-- 
-- Usage:
-- Call the plugin with the sequence name as argument.
-- Example: > Plugin 1 "MySequence" 
-- Triggers the cue picker for sequence "MySequence".
-- If the sequence name contains spaces, enclose it in quotes: > Plugin 1 '"My Sequence"'
---
-- Optionally, you can provide a second argument to assign the selected cue's appearance to a macro.
-- Example: > Plugin 1 "MySequence,3" to trigger the cue picker for sequence "MySequence" and assign the selected cue's appearance to macro 3.
...
--- Example: > Plugin 1 "MySequence,3,Load" to trigger the cue picker for sequence "MySequence", assign the selected cue's appearance to macro 3 and execute the "Load" command.
--- The default command is "Go". 
--- You can also skip the macro number and provide the command directly as second argument.
--- Example: > Plugin 1 "MySequence,Load" to trigger the cue picker for sequence "MySequence" and execute the "Load" command.
---
...
local function main(displayHandle, argument)
    ...
    local seq = ObjectList('Sequence "' .. seqName .. '"')[1]
    if seq == nil then
    ...
    signalTable.CueButtonClicked = function(caller)
        ...
        Cmd(string.format("%s Sequence \"%s\" Cue %d", cmdStr, seqName, cue.No / 1000))
