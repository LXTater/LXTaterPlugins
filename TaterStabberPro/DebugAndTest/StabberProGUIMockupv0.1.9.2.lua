...
function CreateInputDialog(displayHandle)  
  ...
  subTitle.Text = "Set MATricks for Stabs"
  ...
  -- Left side: Minimal recipe sheet (Group, Value, MATricks)
  local recipeGrid = mainContent:Append("UILayoutGrid")
  recipeGrid.Columns = 2
  recipeGrid.Rows = 3  -- Headers + one data row + MATricks sub-section hint
  recipeGrid.Anchors = {
    left = 0,
    right = 0,
    top = 0,
  ...
  headerGroup.Text = "Group (Selection)"
  headerGroup.Anchors = {left=0, right=0, top=0, bottom=0}
  headerGroup.Font = "Medium18"
  headerGroup.TextalignmentH = "Center"
  headerGroup.Padding = "5,5"
  headerGroup.HasHover = "No"

  local headerValue = recipeGrid:Append("UIObject")
  headerValue.Text = "Value (Preset)"
  headerValue.Anchors = {left=1, right=1, top=0, bottom=0}
  headerValue.Font = "Medium18"
  headerValue.TextalignmentH = "Center"
  headerValue.Padding = "5,5"
  headerValue.HasHover = "No"

  --[[
  local headerMatricks = recipeGrid:Append("UIObject")
  headerMatricks.Text = "MATricks Settings"
  headerMatricks.Anchors = {left=2, right=2, top=0, bottom=0}
  headerMatricks.Font = "Medium18"
  headerMatricks.TextalignmentH = "Center"
  headerMatricks.Padding = "5,5"
  headerMatricks.HasHover = "No"
]]
  -- Recipe data row (minimal: one row for inputs)
...
--[[
  local matricksDisplay = recipeGrid:Append("Button")
  matricksDisplay.Text = "Edit MATricks"
  matricksDisplay.Anchors = {left=2, right=2, top=1, bottom=1}
  matricksDisplay.TextalignmentH = "Center"
  matricksDisplay.Padding = "5,5"
  matricksDisplay.PluginComponent = myHandle
  ]]
  -- Add Clicked handler if needed later (e.g., open sub-dialog for MATricks)

  -- Placeholder for MATricks details (minimal, could expand to sub-grid if needed)

  --[[
  local matricksDetails = recipeGrid:Append("UIObject")
  matricksDetails.Text = "(X/Y/Z Groups, Blocks, Wings)"
  matricksDetails.Anchors = {left=0, right=2, top=2, bottom=2}
  matricksDetails.Font = "Medium14"
  matricksDetails.TextalignmentH = "Left"
  matricksDetails.Padding = "5,5"
  matricksDetails.HasHover = "No"
  -- Could replace with a sub-grid mirroring the right side's MATricks if more detail is needed
]]
...
UpdateNumSteps(Interleave)
  ...
  signalTable.SelectPreset = function(caller)
    local groups = Root().ShowData.DataPools.Default.Groups:Children()
    local groupNames = {}
    for _, group in ipairs(groups) do
      table.insert(groupNames, group.name)
